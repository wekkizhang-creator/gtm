...
async function startSmtp(): Promise<{ port: number; messages: string[]; close: () => Promise<void> }> {
  ...
  const server = net.createServer((socket) => {
    ...
    let data = '';
    socket.write('220 undo.smtp.local ESMTP\r\n');
...
async function login(base: string, email: string, smtpMessages: string[]): Promise<string> {
  ...
  const loginRes = await fetch(`${base}/api/auth/login`, {
    ...
    body: JSON.stringify({
      ...
      code,
      agreedToTerms: true,
      device: { deviceId: `completion-undo-${email}`, deviceName: 'Completion undo test', platform: 'Web', appVersion: 'test' },
    }),
  });
  await json(loginRes);
...
async function main() {
  ...
  const dbPath = resolve(root, 'server', 'data', `task-completion-undo-test-${Date.now()}.db`);
  ...
  Object.assign(process.env, {
    ...
    AUTH_TOKEN_SECRET: 'completion-undo-token-secret',
    AUTH_IDENTIFIER_SECRET: 'completion-undo-identifier-secret',
  ...
  try {
    ...
    const alice = await login(base, 'completion-undo-alice@example.com', smtp.messages);
    const bob = await login(base, 'completion-undo-bob@example.com', smtp.messages);

    const created = await req(base, '/api/tasks', {
      method: 'POST',
      cookie: alice,
      body: JSON.stringify({ title: 'Undoable task' }),
    });
    assert(created.res.status === 201, `task create failed: ${created.res.status}`);
    const taskId = created.body.task.id;

    const completed = await req(base, `/api/tasks/${taskId}`, {
      method: 'PATCH',
      cookie: alice,
      body: JSON.stringify({ completed: true }),
    });
    assert(completed.res.status === 200 && completed.body.task.completed === true, 'task should complete before undo');

    const undone = await req(base, '/api/tasks/batch', {
      method: 'POST',
      cookie: alice,
      body: JSON.stringify({ taskIds: [taskId], action: 'update', patch: { completed: false } }),
    });
    assert(undone.res.status === 200 && undone.body.affected === 1, `undo batch failed: ${undone.res.status}`);
    assert(undone.body.tasks[0].completed === false && undone.body.tasks[0].status === 'todo', 'undo should reopen the task');

    const completedView = await req(base, '/api/tasks?view=completed', { cookie: alice });
    assert(!completedView.body.tasks.some((task: any) => task.id === taskId), 'undone task should leave completed view');

    const bobUndo = await req(base, '/api/tasks/batch', {
      method: 'POST',
      cookie: bob,
      body: JSON.stringify({ taskIds: [taskId], action: 'update', patch: { completed: false } }),
    });
    assert(bobUndo.res.status === 404, `another user should not undo Alice task, got ${bobUndo.res.status}`);

    const db = new DatabaseSync(dbPath);
    try {
      const row = db.prepare('SELECT completed, completed_at, status FROM tasks WHERE id = ?').get(taskId) as
        | { completed: number; completed_at: string | null; status: string }
        | undefined;
      assert(row?.completed === 0 && row.completed_at === null && row.status === 'todo', 'SQLite task row should be reopened by undo');
      const activity = db.prepare('SELECT action FROM task_activity_logs WHERE task_id = ? ORDER BY created_at ASC').all(taskId) as Array<{ action: string }>;
      assert(activity.some((item) => item.action === 'task_completed'), 'completion activity should be recorded');
      assert(activity.some((item) => item.action === 'task_reopened'), 'undo should record task_reopened activity');
    } finally {
      db.close();
    }
  } finally {
